...
- Không lưu screenshot/mockup phục vụ test hoặc review thành visual reference bền vững trong Git. Khi owner yêu cầu chụp UI, chỉ tạo ảnh tạm trong `.codex/screenshots/` hoặc artifact ignored tương đương, dùng để kiểm tra/gửi owner rồi dọn khi không còn cần. UI đã chốt phải được ghi bằng rule/pattern ngắn trong `docs/ui-references/approved-patterns.md`, rule rộng trong `docs/11-ui-design-system.md` khi phù hợp và code thật; không dùng `docs/final-screen-ui` hay ảnh raster làm source of truth.
