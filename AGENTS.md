...
| Smart video learning                | `M15.x`   | `AGENTS.md`, `docs/09-implementation-plan.md`, `docs/implementation/M15.md`, `docs/01-product-scope.md`, `docs/02-user-flows.md`, `docs/04-database-model.md`, `docs/database/progress-enrollment.md`, `docs/database/notes-gamification-audit.md`, `docs/05-api-contract.md`, `docs/api/student-learning.md`, `docs/06-ai-rag-spec.md`, `docs/08-ui-pages-and-components.md`, `docs/12-performance-and-observability.md` |
...
- Mọi nút bấm và đường link có thể click được phải có `cursor: pointer` qua CSS base, shared component hoặc class phù hợp. Trạng thái không click được như disabled/loading phải dùng cursor đúng trạng thái (`not-allowed`, `wait`, `default` hoặc tương đương), không để nhìn như đang click được.
- Khi nối hoặc hoàn thiện một UI, mọi tính năng/control/action đang hiển thị và có vẻ dùng được phải có backend API/storage behavior tương ứng trong cùng phạm vi task. Nếu backend chưa làm hoặc nằm ngoài scope, UI phải disable/hide hoặc hiển thị trạng thái chưa khả dụng rõ ràng; không để action client-only giả thành công, upload giả, delete/restore giả, hoặc field lưu local trong flow đã được coi là production-connected.
- Không gom nhiều React component vào một file, kể cả shared primitives hoặc shadcn/Radix wrapper. Mỗi file `.tsx` chỉ nên có một component chính; subcomponent render JSX phải tách file riêng. Không tạo file barrel/re-export chỉ để gom import cho tiện; screen entry `index.tsx` là file màn hình chính, còn route/feature code nên import thẳng file thật. Component con, form control, state view, hook orchestration, schema/DTO, mapper/formatter/helper và mock data phải tách file theo feature/module rõ ràng; file page/manager chỉ nên compose layout và nối state/action cần thiết.
- Trước khi tạo component, hook, API client/service hoặc form control mới, phải kiểm tra component/pattern đã có trong `apps/web/components`, feature tương tự và `docs/ui-references/approved-patterns.md`. Pattern đã được owner ưng phải được tái sử dụng hoặc nâng thành shared component/hook/service; không tạo lại UI/control cùng chức năng với style khác. Component chỉ dùng riêng một màn phải đặt trong `screens/<screen>/components`; phần có thể dùng lại nhiều màn phải đặt ở shared layer rõ ràng.
- Trước khi tạo hoặc sửa bất kỳ form nào, bắt buộc chọn form chuẩn/pattern đã duyệt gần nhất làm tham chiếu: đọc `apps/web/components/common/forms`, form tương tự trong feature hiện có và `docs/ui-references/approved-patterns.md`; trong kế hoạch/final phải nêu đã reuse pattern nào hoặc ghi rõ vì sao chưa có pattern phù hợp.
...
- Chữ trong button không bao giờ được xuống dòng. Mọi button phải giữ text một dòng bằng `white-space: nowrap`/`whitespace-nowrap`; nếu thiếu chỗ thì chỉnh layout, width, font size, padding hoặc copy ngắn hơn, không cho button label wrap.
...
- Modal/drawer phải luôn căn giữa theo chiều dọc và chiều ngang trong viewport ở mọi breakpoint, kể cả mobile. Không top-align modal trên mobile; nếu nội dung dài thì giới hạn `max-height` và chỉ scroll phần body ở giữa.
- Mọi modal/drawer phải có hai đường thoát rõ ràng: nút action `Hủy` trong footer và nút icon `X` để đóng ở header/shell. Trên mobile, footer giữ rule cũ: một action thì full width, mặc định hai action cùng một hàng hai cột. Trên laptop/desktop, button trong footer modal phải co theo nội dung (`max-content`/`w-auto`) và không kéo full width.
- Màu button trong mọi modal/drawer phải đồng nhất theo vai trò action: action chính/lưu dùng cùng màu `primary` trong cùng hệ modal, mặc định là xanh dương (`sky-600`/`sky-700`) với chữ trắng; hủy/đóng dùng trung tính; destructive dùng đỏ. Không để một modal dùng primary màu đen/tối khi các modal cùng flow đang dùng xanh dương, trừ khi có lý do semantic rõ và được ghi lại.
...
- Với UI phức tạp, bắt buộc tách theo flow `app route/page -> feature screens/<screen>/index.tsx -> screen-local components trong screens/<screen>/components -> feature hooks/api/data/schemas/utils -> shared components trong apps/web/components/{common,admin,student,parent}`; không để route/page hoặc screen giữ toàn bộ subcomponent, schema, mock data và helper trong cùng một file.
...
- Khi owner yêu cầu "bỏ field", "không cần field này nữa" hoặc cách nói tương đương, mặc định phải truy vết và loại bỏ field khỏi toàn bộ contract liên quan (UI/form, client type/payload, API/DTO/service, database/migration, worker, test và docs), trừ khi owner nói rõ chỉ ẩn hoặc chỉ đổi UI. Không được suy diễn UI-only chỉ vì yêu cầu kèm screenshot.
...
- Khi sửa CSS/visual nhiều vòng trên cùng màn, đặc biệt màu sắc, font-weight, border, spacing hoặc trạng thái sáng/tối, không được coi `git diff --check` hoặc typecheck là bằng chứng UI đã render đúng trên browser. Các class có thể bị HMR/cache, route sai, component khác, CSS global, route-group CSS, dark mode, token theme hoặc `!important` override. Nếu owner chưa yêu cầu browser/screenshot, final/notification phải nói rõ mức xác nhận là "đã patch code/check tĩnh", không diễn đạt như đã visual-verify. Nếu owner phản ánh "không thấy đổi", bước đầu tiên phải kiểm đúng route/component đang render, dev server/HMR/cache, theme hiện tại và CSS cascade/computed style trước khi tiếp tục đổi class.
...
- Khi owner nói UI đã "ưng/ok/chốt/đúng ý/final" hoặc yêu cầu lưu ảnh UI cuối, Codex phải tự động chụp và lưu screenshot responsive cho route liên quan vào `docs/final-screen-ui/{laptop,ipad,mobile}/{public,student,admin,parent}/{route-path}/`. Mặc định dùng viewport laptop `1440x1000`, iPad `834x1112`, mobile `390x844`, chụp full-page bằng browser/Playwright trên runtime thật khi khả thi. Route `/` lưu ở `home`; route động dùng slug/id thật nếu đang chụp dữ liệu thật. Với route cần auth/data, phải dùng seed/local session đúng role hoặc ghi rõ blocker nếu không dựng được runtime; không được coi UI đã lưu đủ nếu thiếu một viewport hoặc thiếu role/route liên quan mà không nói rõ.
