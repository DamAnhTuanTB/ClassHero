  ...
  const breakdownGroups = useMemo(() => {
    ...
    return [
      ...
      ...Object.values(groups).filter(
        (g) => !["OPENAI", "GEMINI", "MATHPIX"].includes(g.provider),
      ),
    ]
          ...
          <div className="mt-3 space-y-3">
            {breakdownGroups.length ? (
              breakdownGroups.map((group) => {
                const isExpandable =
                  group.provider !== "MATHPIX" && group.modelsList.length > 0;
                const isExpanded = !!expandedProviders[group.provider];

                return (
                  <div
                    key={group.provider}
                    className="rounded-lg bg-[var(--theme-surface-soft)] p-3 space-y-2"
                  >
                    <div
            ...
            <table className="min-w-full text-left text-sm">
              <thead className="bg-[var(--theme-surface-soft)] text-xs uppercase text-[var(--theme-text-muted)]">
                <tr>
                  ...
                  <th className="px-4 py-3">Chi phí lượt gọi</th>
                  ...
                  events.items.map((event) => (
                    ...
                    >
                      ...
                      <td className="px-4 py-3 text-[var(--theme-text)]">
                        <p className="font-extrabold">{formatVnd(event.costVnd)}</p>
                        {hasGenerationCostSummary(event) ? (
                          <p className="mt-0.5 whitespace-nowrap text-xs font-semibold text-[var(--theme-text-muted)]">
                            Tổng {event.aiGeneration.usageEventCount} lượt:{" "}
                            {formatVnd(event.aiGeneration.totalCostVnd)}
                          </p>
                        ) : null}
                      </td>
            ...
            <div className="flex items-center justify-between border-t border-[var(--theme-border)] px-4 py-3">
              ...
              <div className="flex items-center gap-2">
                ...
                <button
                  type="button"
                  disabled={
                    events.pagination.page >= events.pagination.totalPages || isLoading
                  }
                  onClick={() => onEventsPageChange(events.pagination.page + 1)}
                  className="inline-flex h-8 w-8 items-center justify-center rounded-md border border-[var(--theme-border)] bg-[var(--theme-surface)] text-[var(--theme-text-muted)] hover:bg-[var(--theme-surface-soft)] disabled:opacity-50 transition-colors"
                >
            ...
            <div className="space-y-4">
              ...
              <div>
                    ...
                    <tbody className="divide-y divide-[var(--theme-border)] bg-[var(--theme-surface)]">
                        ...
                        <tr>
                          ...
                          <td className="px-4 py-3 text-right text-[var(--theme-text-muted)]">
                            {selectedEvent.priceVersion?.rates?.find(
                              ...
                              ? `$${selectedEvent.priceVersion.rates.find((r) => r.metric === "PAGE")?.unitPriceUsd} / ${new Intl.NumberFormat("vi-VN").format(selectedEvent.priceVersion.rates.find((r) => r.metric === "PAGE")?.unitSize ?? 1)}`
                              : "-"}
                          </td>
                          <td className="px-4 py-3 font-semibold text-right text-[var(--theme-text-strong)]">
                            $
                          ...
                          return (
                            <>
                              <tr>
                                ...
                                <td className="px-4 py-3 text-right text-[var(--theme-text-muted)]">
                                  {selectedEvent.priceVersion?.rates?.find(
                                    ...
                                    ? `$${selectedEvent.priceVersion.rates.find((r) => r.metric === "INPUT_TOKEN")?.unitPriceUsd} / ${new Intl.NumberFormat("vi-VN").format(selectedEvent.priceVersion.rates.find((r) => r.metric === "INPUT_TOKEN")?.unitSize ?? 1)}`
                                    : "-"}
                                </td>
                                <td className="px-4 py-3 font-semibold text-right text-[var(--theme-text-strong)]">
                                  $
                              ...
                              <tr>
                                ...
                                <td className="px-4 py-3 font-semibold text-right text-[var(--theme-text-strong)]">
                                  {new Intl.NumberFormat("vi-VN").format(cachedTokens)}
                                </td>
                                <td className="px-4 py-3 text-right text-[var(--theme-text-muted)]">
                                  {selectedEvent.priceVersion?.rates?.find(
                                    (r) => r.metric === "CACHED_INPUT_TOKEN",
                                  )
                                    ? `$${selectedEvent.priceVersion.rates.find((r) => r.metric === "CACHED_INPUT_TOKEN")?.unitPriceUsd} / ${new Intl.NumberFormat("vi-VN").format(selectedEvent.priceVersion.rates.find((r) => r.metric === "CACHED_INPUT_TOKEN")?.unitSize ?? 1)}`
                                    : "-"}
                                </td>
                                <td className="px-4 py-3 font-semibold text-right text-[var(--theme-text-strong)]">
                                  $
                              ...
                              <tr>
                                ...
                                <td className="px-4 py-3 text-right text-[var(--theme-text-muted)]">
                                  {selectedEvent.priceVersion?.rates?.find(
                                    (r) => r.metric === "OUTPUT_TOKEN",
                                  )
                                    ? `$${selectedEvent.priceVersion.rates.find((r) => r.metric === "OUTPUT_TOKEN")?.unitPriceUsd} / ${new Intl.NumberFormat("vi-VN").format(selectedEvent.priceVersion.rates.find((r) => r.metric === "OUTPUT_TOKEN")?.unitSize ?? 1)}`
                                    : "-"}
                                </td>
                                <td className="px-4 py-3 font-semibold text-right text-[var(--theme-text-strong)]">
                                  $
                                    ...
                                    return (
                                      <div className="flex flex-col items-end gap-0.5">
                                        ...
                                        <div className="text-base text-[var(--theme-primary)] mt-0.5">
                                          $
                                          {selectedEvent.estimatedCostUsd?.toFixed(6) ??
                                            "0.000000"}
                                        </div>
                                      </div>
                                    );
                      ...
                      {selectedEvent.category === "OCR_SERVICE" && (
                        <tr className="bg-[var(--theme-surface-soft)]">
                          <td
                            colSpan={3}
                            className="px-4 py-3 font-bold text-[var(--theme-text-strong)] text-right"
                          >
                ...
                <div className="mt-4 flex flex-col gap-3 rounded-lg bg-[var(--theme-surface-soft)] p-4 border border-[var(--theme-border)] text-sm">
                  ...
                  <div className="flex justify-between items-center">
                    ...
                    <span className="font-semibold text-[var(--theme-text-strong)]">
                      {new Intl.NumberFormat("vi-VN").format(
                        selectedEvent.fxRateVndPerUsd ?? 25000,
                      )}{" "}
                      VNĐ / 1 USD
                    </span>
                  </div>
                  <div className="flex justify-between items-center pt-3 border-t border-[var(--theme-border)] text-[var(--theme-primary)]">
                    ...
                    <span className="font-extrabold text-lg">
                      {formatVnd(selectedEvent.costVnd ?? 0)}
                    </span>
                  </div>
                  {hasGenerationCostSummary(selectedEvent) ? (
                    <div className="flex items-center justify-between border-t border-[var(--theme-border)] pt-3 text-[var(--theme-text-strong)]">
                      <span className="font-bold">
                        Tổng lần sinh ({selectedEvent.aiGeneration.usageEventCount} lượt
                        gọi):
                      </span>
                      <span className="text-lg font-extrabold">
                        {formatVnd(selectedEvent.aiGeneration.totalCostVnd)}
                      </span>
                    </div>
                  ) : null}
                </div>
              ...
              <div>
                ...
                <pre className="text-xs font-mono bg-[var(--theme-surface-soft)] p-4 rounded-lg overflow-auto max-h-96 border border-[var(--theme-border)]">
                  {selectedEvent.rawUsageJson
                    ? JSON.stringify(selectedEvent.rawUsageJson, null, 2)
                    : "Không có dữ liệu"}
                </pre>
              </div>
            </div>
...
function hasGenerationCostSummary(event: UsageEvent): event is UsageEvent & {
