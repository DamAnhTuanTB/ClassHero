...
export function LessonVideoTranscriptPanel({
  ...
  const playbackStartOffset = getPlaybackStartOffset(initialSettings);
  ...
  const [transcriptSourceOffset, setTranscriptSourceOffset] =
    useState(playbackStartOffset);
  ...
  const playbackEndTime =
    playbackWindow?.playbackEndTime ??
    playbackEndTimeInSeconds ??
  ...
  useEffect(() => {
    if (isDirty || hasFetchedDraft) return;

    setTranscriptSourceOffset(playbackStartOffset);
    reset({
      segments: toTranscriptFormSegments(
        initialSettings?.transcript ?? [],
        playbackStartOffset,
  ...
  }, [hasFetchedDraft, initialSettings?.transcript, isDirty, playbackStartOffset, reset]);

  const syncActiveTranscript = useCallback(
    ...
    [playbackEndTime, transcriptSourceOffset],
  );
  ...
  const visibleRows = useMemo(() => {
    ...
    const filteredRows = fields
      ...
      .filter(({ value }) => {
        ...
        const sourceTime = getTranscriptSegmentSourceTime(value);
        return (
          value?.text.toLocaleLowerCase("vi").includes(normalizedQuery) ||
          value?.timeString.includes(normalizedQuery) ||
          value?.sourceTimeString.includes(normalizedQuery) ||
          (sourceTime !== undefined &&
            formatTranscriptTimestamp(sourceTime).includes(normalizedQuery))
        );
      });
    ...
    return filteredRows.map((row, visibleIndex) => {
      ...
      const previousChapter = previousRow
        ...
        : undefined;
      const sourceTime = getTranscriptSegmentSourceTime(row.value);
      ...
      return {
        ...
        isPlaybackTimeAvailable:
          sourceTime !== undefined &&
          isVideoSourceTimeInPlaybackWindow(sourceTime, {
            startTimeInSeconds: transcriptSourceOffset,
            endTimeInSeconds: playbackEndTime,
          }),
        isOutsidePlayback:
          row.value?.sourceTime !== undefined &&
          !isVideoSourceTimeInPlaybackWindow(row.value.sourceTime, {
            startTimeInSeconds: transcriptSourceOffset,
            endTimeInSeconds: playbackEndTime,
          }),
        showChapterHeader:
  ...
  const fetchMutation = useMutation({
    ...
    onSuccess: (draft) => {
      reset({
        segments: toTranscriptFormSegments(draft.segments, draft.playbackStartTime),
      });
      ...
      if (draft.segments.length === 0) {
        toast.warning("Video không có đoạn bản chép lời công khai nào");
      } else {
        toast.success(
  ...
  const onSubmit = (values: TranscriptFormValues) => {
    ...
    const transcript = parsedValues.data.segments
      .map((segment) => {
        const originalSourceTime = segment.sourceTime;
        const sourceTime = roundTranscriptTime(
          originalSourceTime !== undefined &&
            segment.sourceTimeString === formatTranscriptTimestamp(originalSourceTime)
            ? originalSourceTime
            : parseTimestamp(segment.sourceTimeString),
        );
        const originalDuration =
          segment.sourceEndTime !== undefined && originalSourceTime !== undefined
            ? Math.max(0, segment.sourceEndTime - originalSourceTime)
            : undefined;
        return {
          ...(originalDuration !== undefined
            ? {
                endTime: roundTranscriptTime(sourceTime + originalDuration),
              }
            : {}),
          time: sourceTime,
  ...
  return (
    <>
      <section className="mt-4 overflow-hidden rounded-xl border border-[var(--theme-border)] bg-[var(--theme-surface)] shadow-sm">
        ...
        <div className="flex flex-col gap-3 border-b border-[var(--theme-border)] p-4 sm:flex-row sm:items-center sm:justify-between">
          <div className="min-w-0">
            <p className="text-sm font-semibold text-[var(--theme-text-strong)]">
              {fields.length > 0
                ? `${fields.length.toLocaleString("vi-VN")} đoạn của video gốc`
            ...
            <p className="mt-1 text-xs font-medium text-[var(--theme-primary)]">
              Khoảng phát: {formatTranscriptTimestamp(transcriptSourceOffset)}–
              {playbackEndTime === Number.MAX_SAFE_INTEGER
                ? "hết video"
                : formatTranscriptTimestamp(playbackEndTime)}{" "}
              của video gốc
            </p>
            <p className="mt-1 text-xs text-[var(--theme-text-muted)]">
              Các đoạn ngoài khoảng phát vẫn hiển thị nhưng không thể chỉnh sửa.
            </p>
          </div>
          <div className="flex shrink-0 flex-wrap items-center justify-end gap-2">
            ...
            <button
              type="submit"
              form="lesson-video-transcript-form"
              disabled={fields.length === 0 || isBusy || !accessToken}
              className="inline-flex h-10 items-center justify-center gap-2 rounded-lg bg-[var(--theme-primary)] px-5 text-sm font-semibold text-white transition enabled:hover:bg-[var(--theme-primary-hover)] disabled:cursor-not-allowed disabled:opacity-50 dark:text-[var(--theme-primary-foreground)]"
            >
        ...
        <form id="lesson-video-transcript-form" onSubmit={handleSubmit(onSubmit)}>
          {fields.length > 0 && (
            ...
            >
              {visibleRows.map(
                ({
                  ...
                  const isChapterOutsidePlayback = chapter
                    ? !isTranscriptChapterInPlaybackWindow(
                        chapter,
                        draftChapters,
                        transcriptSourceOffset,
                        playbackEndTime,
                  ...
                  return (
                    <Fragment key={field.id}>
                      {showChapterHeader && chapter && (
                        <div
                          aria-disabled={isChapterOutsidePlayback || undefined}
                          className={cn(
                            "sticky top-0 z-20 flex flex-col gap-2 border-b border-[var(--theme-border)] px-4 py-4 shadow-sm sm:flex-row sm:items-center sm:justify-between",
                            isChapterOutsidePlayback
                              ? "bg-[var(--theme-bg-hover)] text-[var(--theme-text-muted)] opacity-80"
                        ...
                        >
                          <div className="flex min-w-0 items-center gap-2">
                            ...
                            {isChapterOutsidePlayback && (
                              <span className="shrink-0 rounded-full border border-[var(--theme-border)] bg-[var(--theme-surface)] px-2 py-0.5 text-[0.65rem] font-bold text-[var(--theme-text-muted)]">
                                Ngoài đoạn phát
                              </span>
                            )}
                          </div>
                          <div className="flex w-full items-center justify-between gap-2 sm:w-auto sm:justify-end">
                            ...
                            <div className="flex shrink-0 flex-wrap justify-end gap-1.5">
                              <span className="w-fit whitespace-nowrap rounded-md bg-[var(--theme-primary)]/15 px-2.5 py-1 text-xs font-bold text-[var(--theme-primary)]">
                                Thời gian{" "}
                                {isChapterOutsidePlayback
                                  ? "—"
                              ...
                              <span className="w-fit whitespace-nowrap rounded-md border border-[var(--theme-border)] bg-[var(--theme-surface)] px-2.5 py-1 text-xs font-bold text-[var(--theme-text-muted)]">
                                Gốc {formatTranscriptTimestamp(chapter.time)}
                              </span>
                            </div>
                          </div>
                        </div>
                      ...
                      <LessonVideoTranscriptRow
                        ...
                        index={index}
                        isBusy={isBusy}
                        isPlaybackTimeAvailable={isPlaybackTimeAvailable}
                        isOutsidePlayback={isOutsidePlayback}
                        ...
                        setValue={setValue}
                        playbackStartTime={transcriptSourceOffset}
                        striped={index % 2 === 1}
          ...
          <div className="flex flex-col gap-3 border-t border-[var(--theme-border)] p-4 sm:flex-row sm:items-center">
            <button
              type="button"
              onClick={() =>
                append({
                  sourceEndTime: undefined,
                  sourceTime: undefined,
                  sourceTimeString: "",
                  timeString: "",
                  text: "",
...
function findActiveTranscriptIndex(
  ...
  for (let index = 0; index < (segments?.length ?? 0); index += 1) {
    ...
    const sourceTime = getTranscriptSegmentSourceTime(segment);
    if (
      sourceTime !== undefined &&
      !isVideoSourceTimeInPlaybackWindow(sourceTime, {
        startTimeInSeconds: playbackStartTime,
        endTimeInSeconds: playbackEndTime,
      })
    ) {
      continue;
...
function toTranscriptFormSegments(
  ...
  return [...segments]
    .sort((first, second) => first.time - second.time)
    .map((segment) => {
      const playbackTime = resolveVideoPlaybackOffsetTime(segment.time, sourceOffset);
      return {
        ...(segment.endTime !== undefined
          ? {
              endTime: resolveVideoPlaybackOffsetTime(segment.endTime, sourceOffset),
              sourceEndTime: segment.endTime,
            }
          : {}),
        sourceTime: segment.time,
        sourceTimeString: formatTranscriptTimestamp(segment.time),
        text: segment.text,
        time: playbackTime,
        timeString: formatTranscriptTimestamp(playbackTime),
      };
    });
}
...
function getTranscriptSegmentSourceTime(
  segment: TranscriptFormValues["segments"][number] | undefined,
) {
  if (!segment) {
    return undefined;
  }
  if (!transcriptTimestampPattern.test(segment.sourceTimeString)) {
    return undefined;
  }

  if (
    segment.sourceTime !== undefined &&
    segment.sourceTimeString === formatTranscriptTimestamp(segment.sourceTime)
  ) {
    return segment.sourceTime;
  }

  return parseTimestamp(segment.sourceTimeString);
}
...
function getActiveTranscriptChapter(
  ...
  return [...chapters]
    ...
    .find((chapter) => chapter.time <= sourceTime);
}

function isTranscriptChapterInPlaybackWindow(
  chapter: VideoChapter,
  chapters: VideoChapter[],
  playbackStartTime: number,
  playbackEndTime: number,
) {
  const chapterAtPlaybackStart = [...chapters]
    .filter((candidate) => candidate.time <= playbackStartTime)
    .sort((first, second) => second.time - first.time)[0];

  return (
    chapter === chapterAtPlaybackStart ||
    isVideoSourceTimeInPlaybackWindow(chapter.time, {
      startTimeInSeconds: playbackStartTime,
      endTimeInSeconds: playbackEndTime,
    })
  );
