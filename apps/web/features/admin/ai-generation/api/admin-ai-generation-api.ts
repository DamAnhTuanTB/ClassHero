...
/**
 * A Test is generated by the Quiz pipeline, but its transport target must
 * remain a Test set. Strip form-only and legacy Quiz fields defensively at
 * the API boundary so preview and submit always have the same contract.
 */
export function serializeAdminAiGenerationPayload(
  ...
  const {
    type: _type,
    targetQuizSetId: _targetQuizSetId,
    durationMinutes: _durationMinutes,
    durationSeconds: _durationSeconds,
    difficultyRatio: _difficultyRatio,
    difficultyRatioJson: _difficultyRatioJson,
    ...body
  } = payload as AdminTestGenerationPayload & {
    difficultyRatio?: unknown;
    difficultyRatioJson?: unknown;
    durationMinutes?: unknown;
    durationSeconds?: unknown;
    targetQuizSetId?: unknown;
  };
  return body;
}
...
export function generateAdminLessonContent(
  ...
  const body = serializeAdminAiGenerationPayload(payload);
...
export function previewAdminTestPrompt(
  lessonId: string,
  payload: AdminTestGenerationPayload,
  token: string,
) {
  const body = serializeAdminAiGenerationPayload(payload);
  return apiRequest<AdminTestPromptPreview>(
    `/admin/lessons/${lessonId}/test-sets/generate-ai/preview`,
    {
      method: "POST",
      body,
      token,
