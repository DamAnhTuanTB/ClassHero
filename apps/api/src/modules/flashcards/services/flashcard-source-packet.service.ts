...
import type {
  FlashcardSourcePacket,
  FlashcardSourcePacketModelManifest,
  FlashcardSourcePacketPage,
} from "#api/modules/flashcards/types/flashcard-source-packet.types";

export type FlashcardSourcePacketErrorCode =
  | "LESSON_NOT_FOUND"
...
export class FlashcardSourcePacketError extends Error {
  constructor(
    readonly code: FlashcardSourcePacketErrorCode,
    message: string,
    readonly details?: unknown,
  ) {
    super(message);
    this.name = "FlashcardSourcePacketError";
  }
}

@Injectable()
export class FlashcardSourcePacketService {
  constructor(
  ...
  async build(
    lessonId: string,
    documentIds: string[],
  ): Promise<FlashcardSourcePacket> {
    const documents = await this.loadDocuments(lessonId, documentIds);
    const output = await PDFDocument.create();
    output.setTitle("Temporary Flashcard source packet");
    output.setAuthor("learning-path-mvp");
    output.setCreator("flashcard-source-packet-v1");
    output.setProducer("flashcard-source-packet-v1");
    ...
    const pages: FlashcardSourcePacketPage[] = [];
    for (let segmentOrder = 0; segmentOrder < documents.length; segmentOrder += 1) {
      ...
      if (!sourcePdf) {
        try {
          ...
          throw new FlashcardSourcePacketError(
            "AI_PDF_SOURCE_NOT_READY",
    ...
    if (pages.length === 0) {
      throw new FlashcardSourcePacketError(
        "AI_PDF_SOURCE_EMPTY",
        "Packet PDF Flashcard phải có ít nhất một trang.",
      );
    }
    const maxPages = this.config.get("AI_FLASHCARD_PACKET_MAX_PAGES", {
      infer: true,
    });
    if (pages.length > maxPages) {
      throw new FlashcardSourcePacketError(
        "AI_PDF_PACKET_TOO_MANY_PAGES",
        `Packet Flashcard có ${pages.length} trang, vượt giới hạn ${maxPages} trang.`,
    ...
    const maxBytes =
      this.config.get("AI_FLASHCARD_PACKET_MAX_MB", { infer: true }) * 1024 * 1024;
    if (bytes.length > maxBytes) {
      throw new FlashcardSourcePacketError(
        "AI_PDF_PACKET_TOO_LARGE",
        "Packet PDF Flashcard vượt giới hạn dung lượng; hãy thu hẹp nguồn đã chọn.",
    ...
    const modelManifest: FlashcardSourcePacketModelManifest = {
      version: 1,
    ...
    const filename = "flashcard-source.pdf";
    const objectKey = `temporary/flashcard-source-packets/${lessonId}/${randomUUID()}-${packetHash.slice(0, 16)}.pdf`;
  ...
  async computeCurrentSourceHash(lessonId: string, documentIds: string[]) {
    return hashAiValue(
      buildSourceSnapshot(lessonId, await this.loadDocuments(lessonId, documentIds)),
    );
  }
  ...
  private async loadDocuments(lessonId: string, documentIds: string[]) {
    ...
    if (uniqueDocumentIds.length === 0 || documents.length !== uniqueDocumentIds.length) {
      throw new FlashcardSourcePacketError(
        "AI_PDF_SOURCE_NOT_READY",
        "Mọi tài liệu Flashcard phải thuộc bài học và ở trạng thái READY.",
    ...
    return documents.map((document) => {
      ...
      if (
        ...
        throw new FlashcardSourcePacketError(
          "AI_PDF_SOURCE_NOT_READY",
          `Tài liệu ${document.title ?? document.file.originalName} chưa đủ điều kiện tạo packet PDF Flashcard.`,
