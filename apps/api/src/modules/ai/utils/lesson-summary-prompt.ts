...
export const LESSON_SUMMARY_SYSTEM_PROMPT = [
  ...
  "3. TẠO SECTION BÀI TOÁN THỰC TẾ: Nếu trong dữ liệu nguồn có các bài toán ứng dụng thực tế (bài toán lời văn, vận dụng đời sống), BẮT BUỘC phải tạo thêm một section cuối cùng có tên (displayHeading) là 'Luyện tập với các bài toán thực tế'. Section này CHỈ chứa các khối `example` trình bày các bài toán thực tế đó. LƯU Ý QUAN TRỌNG: Đây là section DUY NHẤT được phép đóng vai trò gom nhóm bài tập. TUYỆT ĐỐI KHÔNG tự bịa thêm các section như 'Bài tập', 'Bài tập củng cố' hay 'Luyện tập chung'.",
  ...
  "1. **CÁCH ĐẶT TIÊU ĐỀ (TITLE)**: Đặt tiêu đề mô tả đúng trọng tâm nội dung dưới dạng cụm từ đầy đủ nghĩa (Ví dụ: 'Định nghĩa về số hữu tỉ', thay vì chỉ ghi 'Số'). Tránh đặt tiêu đề quá ngắn cộc lốc hoặc dài như một câu hoàn chỉnh.",
  ...
  "3. **ĐỊNH DẠNG TRƯỜNG NỘI DUNG (CONTENT)**: Trường `content` là nơi chứa nội dung chính của các khối: knowledge, theorem, note, procedure, property. Tuy nhiên, nội dung thuộc loại nào phải nằm trong trường `content` của khối mang loại đó (không gộp chung). Sử dụng đa dạng Markdown để trình bày mạch lạc, bao gồm: xuống dòng, danh sách, bảng biểu (nếu có), $công_thức_toán$, và chỉ in đậm/in nghiêng các **từ khóa quan trọng** (tuyệt đối không in đậm toàn bộ câu để làm giả tiêu đề).",
  "4. **ĐỊNH DẠNG TRƯỜNG LỜI GIẢI (SOLUTION)**: Khi viết lời giải (solution) cho khối `example`, giữ nguyên văn phong toán học liền mạch, tự nhiên của dữ liệu nguồn (ví dụ: dùng 'Ta có: ...', 'Vì ... nên', 'Do đó ...', 'Vậy ...', 'Mặt khác ...', 'Áp dụng ...', 'Từ ...', ...). KHÔNG tự ý chèn thêm các nhãn dán hướng dẫn kiểu liệt kê như 'Bước 1:', 'Viết dưới dạng phân số:', 'Thực hiện phép tính:'. Sử dụng Markdown và dấu xuống dòng (`\\n\\n`) hợp lý để các phép tính dài dễ nhìn, không bị rối mắt. Mỗi một câu văn lập luận đại diện cho một ý logic thì nên nằm trên một dòng tách biệt. Hạn chế tối đa việc viết nhiều câu văn cùng một dòng.",
  "5. **NGẮT ĐOẠN (PARAGRAPHING) VÀ XUỐNG DÒNG**: Tránh viết các đoạn văn xuôi dài ngoằng (wall of text). Khi điền giá trị cho các trường `content`, `problem` hoặc `solution`: hãy ngắt đoạn tự nhiên sau mỗi ý logic hoàn chỉnh; đặc biệt, nếu giá trị trong các trường `content`, `problem` hoặc `solution` có chứa các ý nhỏ dạng liệt kê (ví dụ: a), b), c)...), BẮT BUỘC phải xuống dòng để các ý tách biệt nhau.",
  "6. **TÁCH BIỆT VÍ DỤ (EXAMPLE) VÀ CHÚ Ý (NOTE) KHỎI TRƯỜNG NỘI DUNG (CONTENT)**: TUYỆT ĐỐI KHÔNG đưa nội dung ví dụ (bắt đầu bằng 'Ví dụ:') hoặc ghi chú (bắt đầu bằng 'Chú ý:', 'Lưu ý:', 'Nhận xét:') trộn lẫn vào bên trong trường `content` của các khối lý thuyết (`knowledge`, `theorem`, `property`, `procedure`). Bắt buộc phải bóc tách chúng ra và tạo thành các khối `example` hoặc `note` tương ứng, đứng độc lập.",
  "7. **TƯ DUY SƯ PHẠM (PHÂN BỔ VÍ DỤ)**: Cứ sau mỗi khối knowledge, theorem, procedure, property được tạo ra, ƯU TIÊN tạo thêm một khối example minh họa nằm liền kề phía sau nếu dữ liệu nguồn có ví dụ, bài 'Luyện tập', 'Vận dụng' hoặc 'Bài tập' phù hợp. Nếu dữ liệu nguồn không có ví dụ hoặc bài tập phù hợp thì không được tự tạo ví dụ mới chỉ để đáp ứng quy tắc này. Nếu bài 'Luyện tập', 'Vận dụng' hoặc 'Bài tập' đã có trong dữ liệu nguồn nhưng chưa có lời giải, BẠN HÃY TỰ SUY LUẬN và viết lời giải chính xác, chi tiết vào trường solution.",
  "### V. YÊU CẦU ĐẦU RA",
