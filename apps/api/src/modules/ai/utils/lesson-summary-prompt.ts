...
const LESSON_SUMMARY_STRUCTURE_INVARIANT_LINES = [
  ...
  "3. Theory chỉ trình bày kiến thức có trong nguồn, mỗi block tập trung vào một tiểu chủ đề. Không nhét ví dụ, đề bài hoặc lời giải vào knowledge/theorem/property/procedure. Chỉ tạo note khi nguồn thật sự có ý `Chú ý`, `Lưu ý` hoặc `Nhận xét`; note.content KHÔNG bắt đầu bằng các nhãn này vì UI đã hiển thị nhãn khối. Ví dụ trong note là tùy chọn; nếu có phải tự đủ dữ kiện và tuyệt đối không nhắc `Hình x.y`, `hình bên`, `xem hình`, ảnh, URL hoặc chi tiết phụ thuộc hình nguồn. Nếu ý ghi chú chỉ hiểu được khi xem hình nguồn thì không tạo note đó. Nếu bài học thuộc Hình học thì MỌI theory block đều BẮT BUỘC có diagramSpec khác null. Với bài không thuộc Hình học, diagramSpec bắt buộc khi chính khối theory cần hình; nội dung về đồ thị, trục số, mặt phẳng tọa độ, bảng, biểu đồ hoặc sơ đồ luôn được coi là cần hình.",
  ...
  "6d. Tự quyết định đúng một trong hai envelope cho từng diagramSpec. Chỉ dùng `{ kind: INTENT, intent: ... }` khi một family/archetype trong schema biểu diễn ĐẦY ĐỦ mọi điểm, đoạn/nét và quan hệ toán học bắt buộc của problem, solution hoặc theory; khi đó chỉ mô tả dữ kiện ngữ nghĩa, backend compiler sẽ dựng tọa độ, điểm lấy mẫu, vạch chia và vị trí nhãn. Không chọn family/archetype gần nhất nếu phải bỏ bớt dù chỉ một thực thể hoặc quan hệ bắt buộc. Nếu INTENT không biểu diễn đầy đủ, bắt buộc dùng `{ kind: RAW_SPEC, spec: ... }` và tự khai báo toàn bộ hình. Ví dụ: với hình chữ nhật ABCD, M là trung điểm BC và cần hai tam giác ABM, DCM, chỉ dùng QUADRILATERAL/RECTANGLE INTENT khi intent đã khai báo đủ M cùng các đoạn AM, DM; nếu không thì phải dùng RAW_SPEC.",
  "6e. Dùng đúng archetype ngữ nghĩa khi archetype đó đủ khả năng biểu diễn toàn bộ hình: MEASUREMENT_SCALE cho thước/nhiệt kế; BASIC_CONSTRUCTION cho đường–tia–đoạn, trung điểm hoặc vuông góc; REGULAR_POLYGON cho đa giác đều; INVERSE_FUNCTION cho đồ thị y=a/x; TRIANGLE_SIMILARITY cho các trường hợp đồng dạng; NET cho hình khai triển khối. Riêng hình tứ giác ABCD nội tiếp đường tròn tâm O chỉ gồm đường tròn, năm điểm O/A/B/C/D và bốn cạnh AB/BC/CD/DA thì BẮT BUỘC dùng INTENT `ADVANCED_GEOMETRY` + `CIRCLE_RELATIONS` + `CYCLIC_QUADRILATERAL`, với pointLabels theo thứ tự `[O, A, B, C, D]`; compiler tự dựng đầy đủ các thành phần đó. Chỉ dùng RAW_SPEC cho tứ giác nội tiếp khi hình còn bắt buộc có đường chéo, bán kính, góc hoặc số đo góc, tiếp tuyến, điểm/đường phụ hay quan hệ khác ngoài capability vừa nêu. Việc tên archetype có vẻ phù hợp không đủ để chọn INTENT; phải đối chiếu lại mọi điểm, cạnh, đường phụ, marker và quan hệ cần nhìn thấy trước khi trả output.",
  ...
  "9. Chỉ đổi văn phong chuyên biệt cho bài Hình học; bài Số học/Đại số tiếp tục trình bày trực tiếp phép tính và chuỗi biến đổi. Với chứng minh/dựng hình, solution phải bắt đầu thẳng vào mạch lập luận có liên kết bằng `Xét...`, `Ta có...`, `Vì... nên...`, `Suy ra...`, `Do đó...`, `Vậy...`; mỗi chặng logic có thể xuống dòng nhưng KHÔNG biến toàn bộ lời giải thành danh sách bullet/checklist rời rạc. Solution chỉ chứa thân lời giải, không có dòng tiêu đề `Lời giải:`/`Chứng minh:` và không lặp nhãn `Kết luận` vì UI và answer đã sở hữu các phần này.",
  "9a. Chỉ với bài Hình học lớp 7–9 yêu cầu `Chứng minh` hoặc `Chứng tỏ`, `geometryStatement` bắt buộc khác null và là nơi duy nhất chứa bảng GT–KL: `hypotheses` chỉ chép các dữ kiện đã cho trong problem, tuyệt đối không đưa kết quả suy ra hoặc đường phụ được dựng thêm vào GT; `conclusions` ghi chính xác điều phải chứng minh và giữ từng ý riêng nếu đề có nhiều ý. Tuyệt đối không chép `Bảng GT–KL`, `GT:` hoặc `KL:` vào solution. Mọi bài Số học/Đại số, bài Hình học lớp 3–6 và bài Hình học không phải chứng minh chính thức phải trả `geometryStatement=null`.",
...
export const LESSON_SUMMARY_SYSTEM_PROMPT = [
  ...
  "- `note`: chỉ dùng cho ý Chú ý, Lưu ý hoặc Nhận xét có trong nguồn; content đi thẳng vào nội dung và không lặp loại nhãn ở đầu. Có thể không cần ví dụ. Nếu kèm ví dụ thì ví dụ phải tự đủ dữ kiện, không tham chiếu hoặc phụ thuộc hình/ảnh/URL của nguồn.",
  "",
  ...
  "3. Không trộn ví dụ, đề bài, lời giải hoặc ghi chú vào content/purpose/steps của knowledge, theorem, property, procedure. Riêng note.content có thể chứa ví dụ tự đủ dữ kiện, nhưng ví dụ không bắt buộc và không được phụ thuộc hình/ảnh/URL của nguồn; content không mở đầu bằng `Chú ý:`, `Lưu ý:` hoặc `Nhận xét:`.",
  "4. Cách viết `solution` phụ thuộc loại bài nhưng luôn chỉ chứa thân lời giải vì UI tự hiển thị nhãn. Với bài tính Số học/Đại số, bắt đầu trực tiếp bằng phép tính hoặc biểu thức cần biến đổi; trình bày các phép tính và biến đổi theo đúng thứ tự suy luận, xuống dòng khi chuyển sang ý hoặc bước biến đổi mới, và chỉ thêm câu giải thích ngắn khi cần nêu căn cứ. Không chèn tiêu đề thao tác như `Nhóm các số hạng thuận tiện`, `Đổi về phân số`, `Áp dụng công thức` hoặc `Bước 1`. Với bài chứng minh hoặc dựng hình, trình bày thành một mạch suy luận liên kết; mỗi chặng phải nêu rõ dữ kiện hoặc căn cứ và kết quả suy ra, đồng thời tuân theo quy tắc 9–9a của contract bắt buộc.",
...
function gradePresentationInstruction(grade: number) {
  ...
  if (grade <= 9) {
    return `Với bài Số học/Đại số, trình bày trực tiếp từng phép tính và bước biến đổi theo đúng thứ tự suy luận. Với bài chứng minh Hình học, điền bảng GT–KL duy nhất vào geometryStatement; solution chỉ trình bày mạch suy luận liên kết theo chuẩn SGK lớp ${grade}, không chép lại GT–KL.`;
  }
