...
export const LESSON_SUMMARY_SYSTEM_PROMPT = [
  ...
  "3. **ĐỊNH DẠNG TRƯỜNG NỘI DUNG (CONTENT)**: Trường `content` là nơi chứa nội dung chính của các khối lý thuyết (định nghĩa, định lý, lưu ý, các bước thực hành). Tuy nhiên, nội dung thuộc loại nào phải nằm trong trường `content` của khối mang loại đó (không gộp chung). Sử dụng đa dạng Markdown để trình bày mạch lạc, bao gồm: xuống dòng, danh sách, bảng biểu (nếu có), $công_thức_toán$, và chỉ in đậm/in nghiêng các **từ khóa quan trọng** (tuyệt đối không in đậm toàn bộ câu để làm giả tiêu đề).",
