...
export class LessonContentGenerationJobService {
  ...
  queueQuiz(lessonId: string, actorUserId: string, input: {
    questionCount: number;
    difficulty: Difficulty;
    questionTypes?: QuestionType[];
  }) {
    const questionTypes = unique(input.questionTypes ?? ALL_QUESTION_TYPES);
    assertQuestionCountCoversTypes(input.questionCount, questionTypes.length);
    return this.queue(AiGenerationType.QUIZ, lessonId, actorUserId, {
  ...
  queueFlashcards(lessonId: string, actorUserId: string, input: {
    cardCount: number;
    difficulty: Difficulty;
  }) {
  ...
  queueTest(lessonId: string, actorUserId: string, input: {
    questionCount: number;
    durationSeconds: number;
    difficultyRatio: { easy: number; medium: number; hard: number };
    questionTypes?: QuestionType[];
  }) {
    const ratioTotal = input.difficultyRatio.easy + input.difficultyRatio.medium + input.difficultyRatio.hard;
  ...
  private async queue(
    ...
    let snapshot;
    try {
      snapshot = await this.context.snapshot(lessonId);
    } catch (error) {
    ...
    const job = await this.jobs.createAndEnqueue({
      ...
      inputMeta,
      idempotencyKey: ["ai-content", type, lessonId, snapshot.sourceHash, randomUUID()].join(":"),
...
function assertQuestionCountCoversTypes(questionCount: number, typeCount: number) {
  if (questionCount < typeCount) {
    throw badRequestException(
      "AI_QUESTION_COUNT_TOO_SMALL",
      "Số câu phải lớn hơn hoặc bằng số loại câu được yêu cầu",
    );
  }
}
