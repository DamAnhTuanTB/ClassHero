...
export function assertQuestionFigureLatexSource(source: string, errorPrefix: string) {
  ...
  if ((roots?.length ?? 0) !== 1) {
    throw new Error(`${errorPrefix}_SOURCE_ROOT_INVALID`);
  }
}

export function autoRepairQuestionFigureLatexSource(input: {
  source: string;
  subjectKey: string;
...
export function sanitizeQuestionFigureSvg(svg: string, errorPrefix: string) {
  const sanitized = svg
    .replace(/<\?xml[^>]*\?>/giu, "")
    .replace(/<!--([\s\S]*?)-->/gu, "")
    .trim();
