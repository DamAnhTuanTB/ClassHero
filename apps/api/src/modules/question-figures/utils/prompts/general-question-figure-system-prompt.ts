...
const GENERAL_QUESTION_FIGURE_SYSTEM_PROMPT = [
  "Bạn tạo hoặc chỉnh sửa một hình TeX/TikZ minh họa đề bài tiếng Việt.",
  "",
  "### HỒ SƠ MÔN HỌC CỦA HÌNH ĐỀ BÀI",
  ...
  "",
  "### QUY TẮC HÌNH MÔN HỌC CHƯA CÓ PROFILE RIÊNG CỦA ĐỀ BÀI",
  ...
  "- Nếu aiMode=EDIT_CURRENT, trước hết xóa mọi nét/annotation cũ không truy được về whitelist của problem, sau đó mới sửa tối thiểu theo adminInstructions và trả toàn bộ source hợp lệ. Nếu aiMode=REGENERATE, dựng lại chỉ từ problem.",
  ...
  "",
  "- Móng hình trung tính luôn bắt buộc và không bị coi là lộ đáp án. Chỉ cấm thêm quan hệ, trạng thái, annotation hoặc điểm nhấn suy ra; không được vì vậy mà bỏ ranh giới, connector, topology, baseline/trục/tick/zero, marker hay legend mẫu nền.",
  "",
  "### KIỂM CHỨNG CHUYÊN MÔN CUỐI",
  "- Chỉ đối chiếu một lượt source cuối với problem và whitelist dữ kiện trực tiếp: mọi node, vùng, connector, hướng, trạng thái và nhãn phải đúng topology, gắn đúng owner, không thiếu/thừa nội dung mang nghĩa và không gây hiểu sai; sửa trực tiếp source nếu còn lệch.",
].join("\n");

export function buildGeneralQuestionFigureSystemPrompt(
  subject: QuestionFigureSubjectSnapshot,
) {
  return GENERAL_QUESTION_FIGURE_SYSTEM_PROMPT.replaceAll(
    "__SUBJECT_NAME__",
    subject.name,
  );
}
