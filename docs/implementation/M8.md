...
### M8.5: Admin discount code UI

Mode: UI + API

Mục tiêu:

- Có giao diện admin quản lý mã giảm giá.

Phạm vi:

- Discount code list.
- Create/edit/disable discount code.
- Validate form theo type/value/start/end/max uses.
- Hiển thị usage/status nếu API hỗ trợ.

Done khi:

- Admin tạo/sửa/tắt mã giảm giá từ UI.
- UI không tự tính quyền hoặc amount cuối; server vẫn validate.
