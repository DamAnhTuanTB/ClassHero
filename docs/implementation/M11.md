...
- Search, filter và sort của course list phải đồng bộ vào URL để giữ nguyên sau refresh/F5.
...
- Parent xem đúng lộ trình hiệu lực của người con đang chọn.

Phạm vi:

- Reuse effective learning path/progress resolver từ M7.8.
- Hiển thị badge bản cá nhân và cây chương/buổi học read-only.
- Enforce parent-child ownership.

Done khi:

- Parent chỉ xem được bản cá nhân của con đã liên kết.
- Parent không có action chỉnh sửa hoặc URL discovery cho bản cá nhân.
