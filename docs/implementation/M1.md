...
- `lessons.chapter_id` nullable nhưng `learning_path_id` bắt buộc; chapter nếu có phải thuộc đúng learning path.
- Lesson trong chapter có order unique theo chapter; lesson top-level chia sẻ logical order với chapter và service giữ invariant cross-table trong transaction.
