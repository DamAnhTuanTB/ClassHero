...
| User/auth/profile foundation   | `M1.2` Done                                        | foundation `M2.1`, auth `M2.2` Done, profile/RBAC `M2.3` Done | `M2.4` mock UI done, API connect pending | Resend optional for reset email  | `M2.2`, `M2.3`, `M2.4 UI typecheck`, `M14.2` | In progress |
