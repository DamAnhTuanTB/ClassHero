...
| Lesson document upload/PDF     | `M1.2` Done, `M1.3` Done, `M4.2` source document/page/range/kind schema Done | `M4.1` storage/upload foundation partial, `M4.2` source document + page range + primary replacement + supplemental lesson document API Done, job status API Done | `M4.5` source document upload + lesson page mapping UI + supplemental upload | local MinIO dev added, `M4.2` durable job rows, `M4.3`, `M4.4` page-level extract/OCR + lesson/supplement chunking pending | worker tests `M14.1`, API tests `M14.2`                               | In progress |
