...
| Lesson document upload/PDF     | `M1.2` Done, `M1.3` Done, `M4.2` source document/page/range/kind schema Done | `M4.1` storage/upload foundation partial, `M4.2` source document + page range + primary replacement + supplemental lesson document API Done, job status API Done | `M4.5` source document upload + lesson page mapping UI + supplemental upload | local MinIO dev added, `M4.2` durable job rows, `M4.3` BullMQ enqueue + worker status foundation Done, `M4.4` paid OCR artifact/page-level content + `printedPage` mapping + normalized image manifest + artifact audit + visual resolver + primary/supplement chunking Done | `M4.2` document API tests, `M4.3` worker foundation tests, `M4.4` worker artifact/chunking/image-manifest/printed-page/audit/resolver tests, live Mathpix + MinIO artifact/image/audit harness, API typecheck/lint/build; broader worker/API flow tests continue in `M14.x` | In progress |
