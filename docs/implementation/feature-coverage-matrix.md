...
| Lesson document upload/PDF     | `M1.2` Done, `M1.3` Done; source document/page/range and supplemental document schema to add in `M4.2` | `M4.1` storage/upload foundation partial, `M4.2` source document + page range + supplemental lesson document API pending | `M4.5` source document upload + lesson page mapping UI + supplemental upload | local MinIO dev added, `M4.3`, `M4.4` page-level extract/OCR + lesson/supplement chunking pending | worker tests `M14.1`                                                  | In progress |
