...
- Nếu `videoUrl` sau chuẩn hóa khác giá trị đang lưu, kể cả thành `null`, backend
  reset `customVideoSettings.chapters`/`transcript`, bỏ `transcriptLanguage` và
  hard-delete `lesson_video_summaries` hiện hành trong cùng transaction. Các field
  cài đặt player khác được giữ; URL không đổi sau chuẩn hóa không gây reset. Việc
  xóa summary do đổi URL có audit snapshot riêng.
...
- Đọc `customVideoSettings` để tính `playbackStartTime` và `playbackEndTime`, nhưng
  trả đầy đủ chapter và segment theo timestamp nguồn của video. Nếu custom player
  bị tắt, khoảng phát là toàn bộ video.
- Giữ nguyên từng cue caption theo `offset` và `duration` YouTube trả về; chỉ chuẩn
  hóa khoảng trắng và bỏ cue rỗng/trùng hệt liền nhau. Không lọc cue ngoài khoảng
  phát, không chia cue thành bucket hoặc ước tính timestamp từng từ.
...
```json
{
  ...
  "chapters": [{ "time": 913, "title": "2. Đơn thức đồng dạng" }],
  "segments": [
    {
      "time": 913.275,
      "endTime": 919.6,
...
Client dùng metadata khoảng phát để quy đổi cột `Thời gian` về trục sau cắt và
giữ `time`/`endTime` của response cho cột `Thời gian gốc`. Chapter/cue ngoài
khoảng `[playbackStartTime, playbackEndTime)` vẫn render nhưng disabled.
...
- Với Summary AI sinh theo contract hiện tại, provider trả
  `contentJson.data.objectives` dưới dạng field cấp cao: đúng một ý ngắn gọn cho
  mỗi section kiến thức, cùng thứ tự, nêu kiến thức hoặc năng lực trọng tâm của
  section tương ứng. Section `Bài tập vận dụng` không nằm trong danh sách;
  backend giữ nguyên nội dung objective do AI sinh khi map output. Nếu số
  objective lệch số section kiến thức, response vẫn giữ candidate và trả
  `warnings`, `warningDetails` cùng root `reviewIssues` có code
  `OBJECTIVE_SECTION_COUNT_MISMATCH` để admin kiểm tra.
- Trả `phaseOneBlockJsonByPath` cho Summary AI sinh bằng raw snapshot version
  `3`; version cũ không có adapter. Mỗi key là
...
## 9.1. Admin video summary API (`M9.7`, implemented)
...
- Backend resolve lesson + saved `customVideoSettings`, lấy toàn bộ transcript và
  chapter theo timeline video gốc, rồi dựng exact system/user prompt và structured
...
- Structured schema version 9 trả `title`, `objectives[]`, `sections[]` và các
  ...
  không có block `summary`. Validator buộc số objectives bằng số sections,
  section order liên tục, timestamp section/Knowledge/Example không giảm;
  `startSeconds` của Knowledge/Example phải khớp cue transcript thật. Section dùng
  chính xác chapter time khi có chapter, hoặc cue sớm nhất khi AI tự chia section.
  Example không bắt buộc; example phụ thuộc visual thiếu dữ kiện vẫn được
  giữ lại và trả warning cho admin, không làm thất bại toàn lượt sinh. LaTeX còn
  mất cân bằng sau bước chuẩn hóa cũng trả warning `MALFORMED_LATEX`, không chặn
  persist.
- Khi source có chapter, schema khóa số `objectives`/`sections` bằng đúng số
  chapter; backend sắp chapter theo timestamp, bỏ tiền tố thứ tự dư thừa khỏi
  title rồi gán lại `order`, `displayHeading`, `startSeconds` từ chapter cùng vị
  trí và kiểm tra mọi block thuộc đúng khoảng chapter. Khi source không
  có chapter, AI được tự chia section; lúc persist thành công backend ghi ngược
  các section thành `customVideoSettings.chapters` và lưu hash mới trong cùng
  transaction. Client refetch lesson detail sau khi job kết thúc để nhận chapter
  vừa sinh mà không cần tải lại trang.
...
- Validate `contentJson` rồi lưu chỉnh sửa/review status
  ...
  không đọc hoặc ghi `lessons.overview_content_json`. Block-format document hiện
  hành dùng version 6 và không nhận `summary`; khi đọc dữ liệu version cũ, backend
  lọc `summary` khỏi response thay vì xóa hàng loạt dữ liệu đã lưu.
- Khi lưu với `reviewStatus=APPROVED`, backend coi đây là lần admin duyệt
  explicit trên source hiện tại: cập nhật lại các source hash và xóa `staleAt`.

### `DELETE /admin/lessons/:lessonId/video-summary`

Role: `ADMIN`. Xóa cứng bản tóm tắt hiện hành và ghi audit log với snapshot trước
khi xóa; không xóa transcript, chapter, video hoặc Lesson Summary.

### Student delivery (`M15.10`)

`GET /student/lessons/:lessonId` trả thêm `videoSummary` hoặc `null`. Backend chỉ
serialize bản `APPROVED`, chưa xóa và có `staleAt = null`; không trả review
status, source hashes, AI generation ID hoặc metadata quản trị. Quyền đọc tiếp
tục dùng access policy enrollment/trial của lesson.
