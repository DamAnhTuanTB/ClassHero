...
- Tab `Thiết lập mặc định` của `/admin/ai-settings` có bốn nhóm Sinh kiến thức,
  Quiz, Thẻ ghi nhớ và Bài kiểm tra. Mỗi nhóm có hai tab/section cấu hình độc lập
  cho Phase 1 text và Phase 2 ảnh, gồm model chính, model dự phòng, capability
  setting và giới hạn token. Flashcard/Test chưa gọi Phase 2 trong pipeline hiện
  tại nhưng vẫn lưu đủ cấu hình ảnh; UI không gắn nhãn cấu hình ảnh là dùng chung
  hoặc lấy từ model text.
...
- Flashcard/Test giữ review flow text-only riêng. Card Quiz dùng review flow hiện
