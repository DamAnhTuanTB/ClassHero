...
| Admin AI generation panel                 | `M9.8` + reopened `M9.2` + `M9.3` + `M9.7` + `M9.17-M9.18` + `M9.33` | Summary/Video Summary/StemFigure + shared Quiz/Test figure review | `lesson_video_summaries`, `stem_figures`, `quiz_figures`, jobs, AI logs | OpenAI, TeX Live sandbox, R2, Socket.IO/Redis | Năm card generation theo thứ tự `Video`, `Kiến thức`, `Quiz`, `Flashcard`, `Test`; card Video thuộc M9.7 và dùng readiness video + transcript độc lập. Modal Summary có checkbox dùng crop SGK trực tiếp và bỏ qua Stage 2 cho cả lượt; mặc định vẫn AI redraw. Figure compile + validator thành công tự `SUCCEEDED`; mọi figure có xóa/thay ảnh/sinh lại, figure TeX còn có preview-to-source. Figure raster SGK thành công có icon cây đũa mở modal lazy-load; chỉ chọn một tool mỗi lần, làm nét tự preview khi chọn; xóa tự hiển thị `Sau chỉnh sửa` sau mỗi nét, đồng thời giữ canvas/cọ active trên ảnh kết quả để tô liên tiếp, dọn lớp nét đỏ sau mỗi preview và giữ mask tích lũy riêng cho apply; không có nút preview riêng. `Áp dụng` commit riêng lượt hiện tại, giữ modal mở, reset preview/mask và dùng revision vừa tạo làm input; làm nét bỏ chọn tool còn xóa giữ cọ active để tô nhiều lượt liên tiếp. Nhãn tổng figure ẩn khi Summary Phase 1 còn chờ/đang chạy; sau Phase 1, nhãn mở modal xem nhanh toàn bộ ảnh theo thứ tự, không crop asset và theo dõi trạng thái từng ảnh realtime. Lưu/phát hành chỉ chặn placeholder lỗi/initial render; student chỉ nhận R2 asset. Card review Quiz/Test hiển thị asset shared `quiz_figures`; M9.33 dùng một socket Admin chung để invalidate/refetch mọi job/figure/document của lesson, có snapshot khi reconnect và slow polling fallback. |
| Admin Cài đặt AI và OCR                   | `M9.11`, `M9.12`, `M9.19`, `M9.20`, `M9.7`, `M9.6`                   | `M9.10`, `M9.12`, `M9.19`, `M9.20`, `M9.7`, `M9.6`                | provider operations                                                     | `M9.9`, `M4.6`, `M9.12`, `M9.19`, `M9.20`     | Route `/admin/ai-settings`; `Thiết lập mặc định` dùng sidebar tính năng bên trái và panel chi tiết bên phải. Summary/Quiz/Flashcard/Test quản lý riêng `Phase 1 - tạo text` và `Phase 2 - tạo ảnh`, gồm model chính/dự phòng và giới hạn token input/output riêng của từng model trong mỗi phase. `Tóm tắt Video` và `Chat với AI` chỉ có cấu hình text, không dựng Phase 2 ảnh giả; model chính và dự phòng vẫn có đủ giới hạn input/output riêng. `Quản lý model` chỉ quản lý metadata model và bảng giá, không hiển thị giới hạn kỹ thuật/trần token. Không hiển thị secret. Hard-stop tuyệt đối phải phân biệt đã dùng/đang giữ/còn lại, trạng thái đã chặn và lỗi thân thiện.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                          |
| Admin mô phỏng Chat với AI                | `M9.34`                                                              | `M9.34`, reuse `M9.6`, `M9.10`                                    | mở rộng chat session/turn trace + provider usage                        | shared Chat runtime `M9.6`, `M9.9-M9.12`      | Route `/admin/ai-chat`; sidebar item `Chat với AI`; chọn buổi học/một khóa/nhiều khóa, cấu hình riêng từng phiên, inspect input/parameter/token/cost/latency từng lượt. Cấu hình phiên tách model chính/dự phòng thành hai nhóm; mỗi nhóm chỉ hiện temperature hoặc reasoning effort theo capability của model hiệu lực. Cột cấu hình hẹp hơn cột hội thoại; nút `Xem input & chi phí` chỉ nằm dưới phản hồi AI và mở modal rộng riêng, không đặt icon xem input trong composer. Tab default tách card model trả lời `CHAT/TEXT`, card embedding truy xuất và card giới hạn ảnh/quota. Tất cả dùng chung runtime với Student, không dựng admin chat runtime riêng.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                          |
...
- Chat AI hỗ trợ text/LaTeX và ảnh theo default Admin (số ảnh/lượt,
  byte cap, JPEG/PNG/WebP allowlist). Student composer hiển thị quota câu hỏi/ảnh
  thành công còn lại trong ngày; nếu còn `N` ảnh thì cho chọn đủ
  `N` ảnh và chỉ ẩn nút upload sau khi đã chọn đủ quota còn lại;
  icon Chat không hiện trong Test runner. Nếu Test còn đang chạy ở tab khác,
  icon trên các màn học vẫn hiện ở vị trí cũ nhưng dùng disabled state; hover
  hoặc focus mở `ImmediateTooltip` với copy `Bạn không thể sử dụng tính năng này
khi đang làm bài thi!`, không dùng `title` native.
...
- Một danh sách thread chung cho `LIBRARY` và `COURSE`, sort theo hoạt động mới
  nhất; badge scope chỉ là metadata/filter, không chia lịch sử.
- CTA `Tạo cuộc trò chuyện mới`; thread chỉ tạo khi gửi tin đầu.
- Detail tiếp tục thread cũ, message list, text/LaTeX, streaming state và lỗi retry.
- Composer upload/preview/xóa tối đa 5 ảnh JPEG/PNG/WebP, 10 MB/ảnh.
- Context badge, refusal ngoài scope và badge `Chế độ gợi ý` khi Quiz/Flashcard
  đang làm.
- Icon ở Học tập tạo `LIBRARY` và gửi các buổi có CTA `Vào học`/`Học tiếp` đang
  thực sự hiển thị của từng khóa làm danh sách ưu tiên; khóa đã hoàn thành, bảo
  trì hoặc không có lesson tiếp tục không đóng góp ID. Course detail tạo `COURSE`
  và ưu tiên buổi tiếp tục đang có CTA; lesson detail tạo `COURSE` và ưu tiên
  lesson đang mở.
  Các ưu tiên này không thu hẹp scope. Lesson trial, khóa chưa mua và Test runner
  không hiện icon.
- Icon trong Quiz/Flashcard runner và màn review Quiz/Flashcard/Test phải truyền
  đúng câu/thẻ đang hiển thị làm target. Target đổi theo điều hướng câu; câu/thẻ
  hiện tại luôn là ngữ cảnh chính, còn policy hint/full do backend xác minh trạng
  thái đã mở đáp án của từng item.
...
- Cả ba section upload đều tạo request `PROCESSING` để hiển thị trạng thái
  OCR/chunking/embedding; UI Admin hiện không có toggle hoặc nhóm riêng cho
  `STORAGE_ONLY`.
...
- Tab `Thiết lập mặc định` của `/admin/ai-settings` có sidebar bên trái
  gồm sáu nhóm theo thứ tự Tóm tắt Video, Sinh kiến thức, Quiz, Thẻ ghi
  nhớ, Bài kiểm tra và Chat với AI; chọn item nào thì chỉ hiển thị panel
  chi tiết của item đó bên phải. Bốn nhóm Sinh kiến thức, Quiz, Thẻ ghi
  nhớ và Bài kiểm tra có hai tab/section cấu hình độc lập cho Phase 1 text
  và Phase 2 ảnh, gồm model chính, model dự phòng, capability setting và giới hạn
  token; hai phase xếp thành hai hàng card riêng, không đặt cạnh nhau trên
  cùng một hàng. Flashcard/Test chưa gọi
  Phase 2 trong pipeline hiện tại nhưng vẫn lưu đủ cấu hình ảnh; UI không gắn
  nhãn cấu hình ảnh là dùng chung hoặc lấy từ model text. Nhóm Tóm tắt Video
  và Chat với AI chỉ có cấu hình text và không hiển thị Phase 2 ảnh.
...
### 6.12. Admin mô phỏng Chat với AI

Route `/admin/ai-chat`, mở từ item `Chat với AI` trên sidebar và nằm ngay trước
`Cài đặt AI`. Màn laptop-first nhưng phải dùng được trên mobile/tablet.

Màn có hai tab cấp trang:

- `Mô phỏng`: danh sách phiên, chat composer/stream, cột cấu hình và modal inspect.
- `Thiết lập mặc định`: tái sử dụng form/capability/options và đúng query/mutation
  của cấu hình `CHAT/TEXT` tại `/admin/ai-settings`; không copy state hoặc tạo
  endpoint default riêng.

Trong tab `Mô phỏng`:

- Cột trái hiển thị phiên của admin, tìm kiếm, trạng thái scope và action `Phiên
mới`. Tạo phiên chọn một trong `Buổi học`, `Một khóa học`, `Nhiều khóa học`;
  select có search, label môn/lớp và selected chip rõ. Scope khóa sau câu đầu.
- Vùng giữa dùng cùng message renderer, composer, upload ảnh, SSE delta, lời từ
  chối do AI trả như nội dung bình thường, retry/error và LaTeX/mhchem behavior
  của Student Chat; shared component được
  nâng lên boundary dùng chung nếu cần, không chép JSX/stream state thành bản Admin.
  Header hội thoại hiển thị tổng chi phí authoritative của toàn phiên. Footer của
  mỗi assistant message hiển thị gọn chi phí, TTFT và thời gian hoàn tất riêng của
  chính lượt phản hồi đó; các số liệu không cộng dồn từ lượt trước.
- Sau khi chọn một lesson, vùng tạo phiên hiện năm tab context không bắt buộc:
  `Tóm tắt video`, `Sinh kiến thức`, `Quiz`, `Flashcard`, `Test`; tab chưa có nội
  dung Student-visible ở trạng thái disabled. Quiz/Flashcard/Test dùng custom
  select để chọn bộ, radio để chọn item và radio policy. Quiz/Flashcard có
  `HINT_ONLY | FULL_ANSWER`; Test có `BLOCKED | FULL_ANSWER`. Không chọn tab hoặc
  bấm `Bỏ ngữ cảnh` vẫn chat theo scope đã chọn; với `COURSE`, lesson hiện tại chỉ
  là retrieval boost. Context tiếp tục xuất hiện trong phiên đã tạo để đổi cho
  lượt sau; khi mở lại phiên `COURSE`, UI khôi phục lesson của lượt gần nhất và
  vẫn cho đổi hoặc bỏ lesson cho lượt kế tiếp, không sửa snapshot lượt cũ.
- Cột phải có `Cấu hình phiên`: default version đang kế thừa, model chính/dự
  phòng, field `temperature` hoặc `reasoningEffort` theo capability, giới hạn
  input/output và preview `Giá trị hiệu lực`. Admin lưu giữa các câu; config mới
  chỉ áp dụng cho câu tiếp theo.
- Tại mỗi assistant message đã có phản hồi có action `Xem input & chi phí`; composer
  không có icon mắt. Action mở modal rộng riêng hiển thị
  system input, user input, history gần, RAG chunks/source, ảnh tham chiếu, exact
  effective parameters, provider/model/request ID, token input/cache-write/cache/
  output/reasoning, chi phí, latency và time-to-first-token. Nội dung dài có vùng
  scroll/copy riêng; system prompt, user prompt và context chunks có các tab preview
  dễ đọc, từng chunk đóng/mở độc lập để duyệt nhanh; secret, signed URL, base64 và
  object key không xuất hiện.
- Desktop dùng split workspace có chiều rộng co giãn và cột cấu hình hẹp hơn cột
  hội thoại; tablet/mobile chuyển danh sách phiên và cấu hình sang drawer/sheet,
  giữ composer/message là vùng chính. Modal inspect dùng cùng kích thước thoải mái
  trên mọi viewport.
- Tên phiên mặc định là câu đầu và hiện optimistic ngay khi bấm gửi;
  `started.title` xác nhận lại, còn `title_updated` cập nhật header/list trong lúc
  phản hồi đầu đang stream, không chờ refetch toàn trang. Lỗi trước `started` trả
  header về trạng thái phiên mới.
- Loading dùng skeleton theo ba vùng; empty state hướng dẫn tạo phiên; lỗi fetch
  dùng `AdminDataErrorState`; streaming/reconnect/failed/budget-blocked có trạng
  thái riêng và không làm mất message đã hoàn tất.

Pattern triển khai phải tham chiếu admin shell hiện có,
`ModelConfigurationsTab`, `EditorDialogShell`, shared Student Chat components và
`AdminDataErrorState`. Đây là UI mới chưa được owner duyệt hình ảnh; chỉ ghi
contract hành vi, chưa ghi vào approved UI patterns trước `/accept-ui`.
