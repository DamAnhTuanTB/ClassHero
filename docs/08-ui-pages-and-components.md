...
| Admin AI generation panel                 | `M9.8` Done            | `M9.2`, `M9.3`                    | `M1.4`, `M1.5`            | `M5.x`, `M9.1`                                | Bốn card Summary/Quiz/Flashcard/Test trong lesson detail; custom form, validation realtime, polling durable job, mở đúng editor/review; front-end không gọi AI trực tiếp. M9.2 đọc summary v1/v2, cho admin sửa trực tiếp block/JSON và render `diagramSpec` an toàn. Block/hình reviewable vẫn hiện phần dùng được, kèm badge `Cần kiểm tra`, câu `Vấn đề`, `Gợi ý sửa`, chi tiết kỹ thuật thu gọn và nút chấp nhận local. Hình không thể dựng mới hiện placeholder `Hình lỗi`, không có nút chấp nhận và có nút đỏ `Xóa hình lỗi`; xóa chỉ đổi bản nháp local, reload trước khi Lưu khôi phục dữ liệu cũ, chỉ `Lưu nội dung` mới persist. Issue không khóa editor/lưu nháp nhưng khóa `Phát hành` tới khi được xử lý. Student chỉ thấy summary đã phát hành và không thấy metadata review. |
...
- Với Summary, `Lưu nội dung` chỉ lưu phần chỉnh sửa thành bản nháp; nếu Summary
  ...
  chưa có action thu hồi. Action `Sinh lại` mở modal cấu hình Summary với cùng
  dữ liệu mặc định như lần `Tạo mới` đầu tiên, không kế thừa cấu hình của job
