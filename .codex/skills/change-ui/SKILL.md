...
- When the owner asks to add a new button or icon beside an existing action, preserve the existing control and its handler exactly, then add a separate control with its own accessible label and handler. Do not repurpose or overwrite the existing icon unless the owner explicitly asks to replace it.
